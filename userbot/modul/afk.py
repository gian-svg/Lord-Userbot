...
# ========================= CONSTANTS ============================
AFKSTR = [
    f"**My Guide🐼 {ALIVE_NAME} Sedang OFF!**",
    f"**Maaf Guide🐼 {ALIVE_NAME} Sedang OFF\n Tunggu Sampai Dia Kembali Online!**",
    f"**Guide🐼 {ALIVE_NAME} Sedang Off\n ➥ Mohon Menunggu Sampai Dia Kembali Online**",
    f"**Maaf yah Guide🐼 {ALIVE_NAME} Masih OFF!**",
]
...
async def type_afk_is_not_true(notafk):
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.respond("**My Guide🐼 Kembali Online**")
        time.sleep(3)
        await msg.delete()
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"**𓆤
 Bee {ALIVE_NAME} Lagi AFK** {afk_since} **Yang Lalu.**\
                        \n➠ **Alasannya:** `{AFKREASON}`")
                else:
                ...
                if USERS[mention.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await mention.reply(f"**🐼 My Guide🐼 Masih AFK** {afk_since} **Yang Lalu.**\
                            \n☣ **Alasan:** `{AFKREASON}`")
                    else:
                        await mention.reply(str(choice(AFKSTR)))
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"🐼 **Guide🐼 Sedang AFK** {afk_since} **Yang Lalu**.\
                        \n☣ **Alasan**: `{AFKREASON}`")
                else:
